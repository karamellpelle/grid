...
makePathOutput size = io $ do
    vbo <- bindNewBuf gl_ARRAY_BUFFER
    glBufferData gl_ARRAY_BUFFER (fI $ size * 128) nullPtr gl_DYNAMIC_DRAW
    return  PathOutput { pathoutputGLVBO = vbo }
...
writePathOutput :: PathOutput -> UInt -> UInt -> Segment -> IO PathOutput
writePathOutput po size end (Segment (Node x y z) (Turn a0 a1 a2 _ _ _ _ _ _)) = do
    let off = fI $ end * 128
