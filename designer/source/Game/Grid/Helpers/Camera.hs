...
cameraToNode :: Camera -> Node -> Camera
cameraToNode camera node' =
    if cameraNodeIdeal camera == node'
        then camera 
        else camera
             {
...
cameraEatCameraCommand cam cmd =
    let turn = if cameracommandTurnIsAdd cmd
               then cameracommandTurn cmd `mappend` cameraCurrentTurn cam
...
cameraToPathTurn camera path = 
    let turn = cameraCurrentTurn camera
        turn' = pathTurn path
    in if turn == turn'
       then camera
            ...
            in  camera
                {
                    ...
                    cameraTurnASpeed = pathSpeed path * valueModePathTurnSpeed,
                    cameraTurnBSpeed = pathSpeed path * valueModePathTurnSpeed,
...
cameraSetPath camera path =
    camera 
    {
        ...
        cameraTurnBSpeed = valueModePathTurnSpeed * pathSpeed path,
        cameraTurnA = mempty,
        cameraTurnAIdeal = mempty,
        cameraTurnAAlpha = 1.0,
        cameraTurnASpeed = valueModePathTurnSpeed * pathSpeed path
    }
...
cameraCurrentTurn :: Camera -> Turn
cameraCurrentTurn camera =
    cameraTurnBIdeal camera `mappend` 
    cameraTurnAIdeal camera 
...
cameraToViewTicks' :: Camera -> TickT -> View -> Camera
cameraToViewTicks' camera ticks view = 
