...
defaultStepDT :: Collision s b ->
                 TickT -> s -> GridWorld -> b -> MEnv' (s, GridWorld, b)
defaultStepDT col dt = \s grid b ->
    ...
    io $ do
      let dt' = rTF dt
          path' = stepDTPath dt' $ gridPathA grid
...
cameraStepDT :: TickT -> s -> GridWorld -> b -> MEnv' (s, GridWorld, b)
...
data Collision s b =
    ...
    {
        onPathNode :: 
          Path -> s -> GridWorld -> b -> IO (Path, s, GridWorld, b)
