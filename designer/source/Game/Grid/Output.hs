...
-- how to typically output GridWorld:
-- 
-- 0. setup screen:                                             done by Scene
-- 1. define projection matrix (a view property of GL camera):  mat4Ortho/mat4Perspective
-- 2. define modelview matrix (GL camera placement):            mat4SceneXYZ/mat4SceneCamera
-- 3. set shaders from properties, and draw
...
-- | get modelview for scene looking at x-direction
mat4SceneXYZ :: GridWorld -> Mat4
mat4SceneXYZ grid = 
    mempty



-- | get modelview for scene from camera. (camera turn is direction)
mat4SceneCamera :: GridWorld -> Mat4
mat4SceneCamera grid =
    let 
        
        -- view: 
        camera = gridCamera grid
        View a b c = cameraView camera
        ...
        matView = mat4ViewABC (tau * 0.25 + a'') b'' c''
    ...
    in   matView `mappend` matTurnB `mappend` matTurnA `mappend` matTranslate
