...
--
module Game.LevelPuzzleMode.Output.Fancy.State
  (
    stateSetComplete,
    stateStepComplete',
...
stateStepSpecialComplete' s lvl run = do
    ...
    run' <- modifyRunIO run $ \state -> 
            ...
            in  return state
                       {
                          Run.outputstateColorIx = outputstateCompleteColorIx s,
                          Run.outputstateColorIx' = outputstateCompleteColorIx' s,
                          Run.outputstateAlpha = outputstateCompleteAlpha s
                       }
    return (s, lvl', run')
...
step :: TickT -> OutputState -> IO OutputState
step tick s = do
...
modifyRunIO :: RunWorld -> (Run.OutputState -> IO Run.OutputState) -> IO RunWorld
