...
--
module Game.LevelPuzzleMode.Do.Grid.StepDT
  (
    collisionPlay,
    collisionComplete,
        ...
        onPathNode = \path s grid lvl -> do
            ...
            handleCollisions path' lvl >>= \(path'', lvl') -> 
                ...
                case levelpuzzleSegmentsCount lvl' of
                    ...
                    -- last segment used => EventNullSegments
                    0   -> case worldPushEvent lvl' EventNullSegments of
                        lvl'' -> return (path'', s, grid, lvl'')

                    n   -> case lvl' { levelpuzzleSegmentsCount = n - 1 } of
                        lvl'' -> return (path'', s, grid, lvl'')
...
handleCollisions :: Path -> LevelPuzzleWorld -> 
                    IO (Path, LevelPuzzleWorld)
...
checkColDotPlain :: Path -> LevelPuzzleWorld -> Room -> 
                    IO (Path, LevelPuzzleWorld)
checkColDotPlain path lvl room = 
    ...
    where
      ...
      levelpuzzleEat lvl dot = do
          ...
          let cnt = levelpuzzleContent lvl
              path = gridPathA $ contentGrid cnt
          ...
          -- update content
          return $ levelpuzzleModifyContent lvl $ \cnt -> 
                   cnt
                   {
...
checkColDotBonus :: Path -> LevelPuzzleWorld -> Room -> 
                    IO (Path, LevelPuzzleWorld)
...
checkColDotTele :: Path -> LevelPuzzleWorld -> Room -> 
                    IO (Path, LevelPuzzleWorld)
...
checkColDotFinish :: Path -> LevelPuzzleWorld -> Room -> 
                    IO (Path, LevelPuzzleWorld)
checkColDotFinish path lvl room = 
    ...
    where
      ...
      levelpuzzleEat lvl dot =
          return $ worldPushEvent lvl EventPathEatDotFinish
          

--------------------------------------------------------------------------------
--  Wall

checkColWall :: Path -> LevelPuzzleWorld -> Room -> 
                IO (Path, LevelPuzzleWorld)
checkColWall path lvl room = 
    ...
    where
          ...
          p ->  let x = wallX wall 
                ...
                in  nodeInner p n == 0 && 0 <= ix && ix <= nodeInner x x
                                       ...
                                       && if wallIsDouble wall 
                                          then True
                                          else isNotWallDir (pathTurn path) n
      isNotWallDir (Turn a0 a1 a2 _ _ _ _ _ _) (Node n0 n1 n2) =
          fI a0 * n0 + fI a1 * n1 + fI a2 * n2 <= 0
      ...
      levelpuzzleEat lvl wall = 
          return $ worldPushEvent lvl EventPathEatWall
