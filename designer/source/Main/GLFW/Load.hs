...
mainLoad path = do
    ...
    case findInvalidObject edit of
        ...
        Just (room, node, str)  -> do
            putStrLn $ "warning        : level invalid at RoomIx: " ++ show room ++ ", " ++ show node ++
                       " (" ++ str ++ ")"

   
    putStrLn $ "name is " ++ (levelName $ editLevel edit) ++ ". rename? "
